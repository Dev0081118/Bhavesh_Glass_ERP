...
export default function SaleBillFilters({
  filters,
  setFilters,
}) {
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-3 text-sm outline-none transition placeholder:text-slate-400 focus:border-slate-400"
          />
        </div>
        ...
        >
          {saleStatuses.map((status) => (
            <option key={status} value={status}>
              {status === "All" ? "All Bill Statuses" : status}
            </option>
          ))}
        </select>
