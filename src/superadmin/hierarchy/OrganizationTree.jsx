...
export default function OrganizationTree({
  ...
  const getId = (person) => {
    ...
    // MongoDB ObjectId directly
    if (person._id) {
      if (typeof person._id === "object") {
        if (person._id.$oid) {
          return String(person._id.$oid);
        }

        if (person._id.toString) {
          return String(person._id.toString());
        }
      }
  ...
  const getManagerId = (person) => {
    if (!person) return "";

    const value = person.managerId;

    if (!value) return "";

    // If managerId is MongoDB extended JSON:
    // { $oid: "..." }
  ...
  // Normalize staff
  // -----------------------------------------
  const normalizedStaff = staff.map((person) => ({
    ...person,
    id: getId(person),
    managerId: getManagerId(person),
  }));

  const admins = normalizedStaff.filter(
    (person) =>
      String(person.role || "").toLowerCase() === "admin"
  );

  const managers = normalizedStaff.filter(
    (person) =>
      String(person.role || "").toLowerCase() === "manager"
  );

  const employees = normalizedStaff.filter(
    (person) =>
      String(person.role || "").toLowerCase() === "employee"
  );
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="text-xs text-slate-400">
            Select a department to view its reporting structure.
          </p>
        </div>
                ...
                {expanded && (
                  <div className="space-y-4 border-t border-slate-200 p-4">
                    {departmentManagers.length === 0 ? (
                      <EmptyMessage text="No manager assigned" />
                    ) : (
                          ...
                          >
                            ...
                            <div className="flex items-start gap-2">
                              <button
                                ...
                                className="mt-4 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50"
                                aria-label={`${
                                  managerExpanded
                                    ? "Collapse"
                                    : "Expand"
