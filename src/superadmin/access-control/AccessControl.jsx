...
const createInitialPermissions = (staff) => {
  ...
  staff.forEach((person) => {
    ...
    permissions[person.id] = {
      ...
      profile: {
        /*
         * Viewing the user's own basic profile should
         * always be allowed.
         */
        view: true,
         ...
         * Current Profile.jsx doesn't provide
         * profile editing yet.
         */
        edit:
          person.access?.profile?.edit ??
          role === "admin",

        /*
         * This permission is actually used
         * by Profile.jsx.
         */
        resetPassword:
    ...
    moduleSource.forEach(
      (moduleName) => {
        const key =
          normalizeModuleKey(moduleName);

        if (
          typeof permissions[person.id][
            key
          ] !== "boolean"
        ) {
...
export default function AccessControl({
  ...
  const updateProfilePermission = (
    ...
    /*
     * Basic profile access should always
     * remain available.
     */
    if (permission === "view") {
      return;
